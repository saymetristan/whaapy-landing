...
function StatCell({ value, label }: { value: string; label: string }) {
  ...
  return (
    <motion.div
      ...
      className="bg-surface px-5 py-5 text-center shadow-premium"
    >
      <p className="font-display text-xl font-bold tracking-tight text-text md:text-2xl">{value}</p>
    ...
    <section className="relative overflow-hidden pt-28 md:pt-36">
      ...
      <div className="container-page relative pb-16 md:pb-24">
        <div className="grid items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-10">
          ...
          <motion.div {...fade(0.25)} className="relative min-h-[320px] lg:min-h-[420px]">
            <div
              aria-hidden
              className="pointer-events-none absolute -inset-6 -z-10 md:-inset-10"
            >
              <div className="absolute left-1/2 top-1/3 h-[320px] w-[min(100%,520px)] -translate-x-1/2 rounded-full bg-primary/20 blur-[100px]" />
            </div>

            <div className="perspective-card relative mx-auto w-full max-w-2xl lg:mr-0 lg:ml-auto">
              <Tilt max={5} className="relative aspect-[16/10] w-full" glare>
